...
import chatbot
import script
from EDA import word_frequency_analysis
from EDA import word_length_analysis
...
if selected == 'EDA' and file:
  if st.session_state['output_for_eda'] == None:
    st.write('Make a request in the chatbot')

  # Word Length Analysis
  elif st.session_state['output_for_eda']['task'] == 'wla':
  ...
  elif st.session_state['output_for_eda']['task'] == 'wfa_wc':
  ...
  elif st.session_state['output_for_eda']['task'][:8] == 'wfa_top_':
    wfa = word_frequency_analysis.WordFrequencyAnalysis(st.session_state['df'],
                                                        st.session_state['output_for_eda']['first_column'])
  ...
  # Lineplot
  elif st.session_state['output_for_eda']['task'] == 'line':
    x = st.session_state['output_for_eda']['first_column']
    y = st.session_state['output_for_eda']['second_column']
    hue = None
    if 'hue' in list(st.session_state['output_for_eda'].keys()):
      hue = st.session_state['output_for_eda']['hue']
  ...
  # Boxplot
  elif st.session_state['output_for_eda']['task'] == 'box':
    y = st.session_state['output_for_eda']['first_column']
    x = None
    if 'hue' in list(st.session_state['output_for_eda'].keys()):
      x = st.session_state['output_for_eda']['hue']
    sns.boxplot(data = st.session_state['df'],
                x = x, 
                y = y)
    st.pyplot()

  # Scatterplot
  elif st.session_state['output_for_eda']['task'] == 'scatter':
    x = st.session_state['output_for_eda']['first_column']
    y = st.session_state['output_for_eda']['second_column']
    hue = None
    if 'hue' in list(st.session_state['output_for_eda'].keys()):
  ...
  # Histogram
  elif st.session_state['output_for_eda']['task'] == 'hist':
    x = st.session_state['output_for_eda']['first_column']
    hue = None
    if 'hue' in list(st.session_state['output_for_eda'].keys()):
      hue = st.session_state['output_for_eda']['hue']
    sns.histplot(data = st.session_state['df'],
                 x = x,
                 hue = hue)
    st.pyplot()
    
  # Barplot
  elif st.session_state['output_for_eda']['task'] == 'bar':
    x = st.session_state['output_for_eda']['first_column']
    hue = None
    if 'hue' in list(st.session_state['output_for_eda'].keys()):
      hue = st.session_state['output_for_eda']['hue']
    sns.countplot(data = st.session_state['df'],
                 x = x,
                 hue = hue)
    st.pyplot()
